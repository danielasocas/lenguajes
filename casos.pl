...
% Caso10:
% Nodo con dos hijos, estos hijos tienen a su vez dos hijos mas, y el hijo derecho del hijo izquierdo tiene un hijo
prueba10 :- bienEtiquetado(nodo(8,[arista(7,nodo(1,[arista(6,nodo(7,[])),arista(5,nodo(6,[arista(4,nodo(2,[]))]))])),arista(3,nodo(5,[arista(2,nodo(3,[])),arista(1,nodo(4,[]))]))])),
...
% Caso13:
% Nodo con un solo hijo, Y ese hijo tiene otro hijo, con nodos repetidos y V != {1,2,3}
...
prueba14 :- \+ bienEtiquetado(nodo(3,[arista(5,nodo(1,[])),arista(1,nodo(2,[]))])),
			write('Caso 14: RECHAZADO \n').

% Caso15:
% Nodo con dos hijos, Y el hijo izquierdo tiene un solo hijo, que es mayor que N
prueba15 :- \+ bienEtiquetado(nodo(1,[arista(2,nodo(3,[arista(1,nodo(2,[]))])), arista(3,nodo(100,[]))])),
			write('Caso 15: RECHAZADO \n').

% Caso16:
% Nodo con dos hijos, Y el hijo derecho tiene un hijo, el hijo izquierdo mayor que N
prueba16 :- \+ bienEtiquetado(nodo(1,[arista(3,nodo(90,[])), arista(2,nodo(3,[arista(1, nodo(2,[]))]))])),
			write('Caso 16: RECHAZADO \n').

% Caso17:
% Nodo con tres hijos, esos hijos no tienen mas hijos (ejemplo proyecto), con raiz 80
prueba17 :- \+ bienEtiquetado(nodo(80,[arista(1,nodo(3,[])),arista(2,nodo(2,[])),arista(3,nodo(1,[]))])),
			write('Caso 17: RECHAZADO \n').

% Caso18:
% Nodo con tres hijos, tercer hijo tiene tres hijos mas, esos hijos no tienen mas hijos, con etiquetas de aristas 1 y 5 intercambiadas
prueba18 :- \+ bienEtiquetado(nodo(4,[arista(5,nodo(3,[])),arista(2,nodo(2,[])),arista(3,nodo(1,[arista(6,nodo(7,[])), arista(1,nodo(6,[])), arista(4,nodo(5,[]))]))])),
			write('Caso 18: RECHAZADO \n').

% Caso19:
% Nodo con dos hijos, hijos tienen dos hijos cada uno, Y esos hijos no tienen mas hijos
prueba19 :- \+ bienEtiquetado(nodo(7,[arista(6,nodo(1,[arista(5,nodo(6,[])), arista(4,nodo(5,[]))])), arista(3,nodo(4,[arista(2,nodo(2,[])), arista(1,nodo(3,[]))]))])),
			write('Caso 19: RECHAZADO \n').

% Caso20:
% Nodo con dos hijos, estos hijos tienen a su vez dos hijos mas, y el hijo derecho del hijo izquierdo tiene un hijo
prueba20 :- \+ bienEtiquetado(nodo(8,[arista(7,nodo(1,[arista(6,nodo(7,[])),arista(5,nodo(6,[arista(4,nodo(2,[]))]))])),arista(3,nodo(5,[arista(2,nodo(3,[])),arista(1,nodo(4,[]))]))])),
